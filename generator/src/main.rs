...
struct Args {
    /// The CSS file generated by the `tailwindcss` tool.
    #[clap(short, long)]
    css: PathBuf,
    ...
    #[clap(short, long)]
    to: PathBuf,
    #[clap(short, long, arg_enum, default_value_t = Visibility::Private)]
...
fn main() {
    let args = Args::parse();
    let structs = structs_from_css_file(&args.css);
    ...
    code.push_str(&modifiers(visibility));

    for group in structs.keys().sorted() {
        let struct_name = group.to_upper_camel_case();
        let mut def = format!(
            "#[derive(Clone, Copy)]\n{} struct {} {{\n",
            visibility, struct_name
        );
        for field in structs.get(group).unwrap().keys().sorted() {
            let f_def = format!(
                "  {} {}: &'static str,\n",
                visibility,
                maybe_escape_field(field),
            );
            def.push_str(&f_def);
        }
        def.push_str("}\n");
    ...
    for group in structs.keys().sorted() {
        ...
        let mut init = format!(
            "{} const {}: {} = {}{{\n",
            visibility, const_name, struct_name, struct_name,
        );
        let members = structs.get(group).unwrap();
        for field in members.keys().sorted() {
            let f_init = format!(
                r#"    {}: "{}",{}"#,
                maybe_escape_field(field),
                members.get(field).unwrap(),
                "\n",
            );
            init.push_str(&f_init);
        }
        init.push_str("};\n");
        code.push_str(&init);
        code.push('\n');
    ...
    write(&args.to, &code).expect("Could not write generated code");

    if args.rustfmt {
        Command::new("rustfmt")
            .arg(&args.to)
            .output()
            .expect("Could not run rustfmt");
    }
}

fn structs_from_css_file(file: &Path) -> HashMap<&'static str, HashMap<String, String>> {
    let content = read_to_string(file).expect("Could not read css file");
...
fn modifiers(visibility: &str) -> String {
    let mut modifiers = format!(
        "#[derive(Clone, Copy)]\n{} struct Modifiers {{\n",
        visibility
    );
    for mod_name in MODIFIERS {
        let field = class_to_rust_var(mod_name);
        let def = format!("    {} {}: &'static str,\n", visibility, field);
        modifiers.push_str(&def);
    }
    modifiers.push_str("}\n\n");
    ...
    for mod_name in MODIFIERS {
        let field = class_to_rust_var(mod_name);
        let def = format!(r#"   {}: "{}",{}"#, field, mod_name, "\n");
        modifiers.push_str(&def);
    }
    modifiers.push_str("};\n\n");
